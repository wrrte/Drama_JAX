...
큰 가치 추락 혹은 상승이 일어나면, 상상을 할 때 해당 프레임과 유사 프레임들이 context 의 마지막에 오도록 배치함. 그렇다면 유사한 상황에서 미래를 상상해나가니 비슷한 상황일 테고, 대신 트리거가 된 놈이 있는 만큼 모델이 자연스럽게 차이를 찾는 방향으로 학습될 것임 (단순히 뭉개지지 않게 하려면 어떡하지? worldmodel이 아니라 actorcritic인 만큼 기본적으로 안 뭉개지나?)
이 과정에서 기존의 샘플링 확률 정책을 최대한 위배하지 않아야 함.

구현 시 prior, posterior 고려하여 가치 변화를 유발한 프레임의 위치를 잘 추적해야 함

현재 actor critic 의 학습 결과가 인코더에 gradient를 가하나? 그렇지 않으면 작은 정보를 강조하기는 좀 힘들어 보여서.
